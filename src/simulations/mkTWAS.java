...
//				}
	...
	public static void get_result(String out_folder) 
		...
		BufferedReader br_info = new BufferedReader(new FileReader(out_folder+"mkTWAS.result"));
		while ((line = br_info.readLine()) != null) {
			 line= line.replace("\n", "").replace("\r", "");
			 String[] tmp_arr = line.split("\t");
...
//		System.out.println("-------------------------------------");
		System.out.println("The p-value of mkTWAS on GENE "+ Gene_ENSG_ID+":\t"+ 
...
//        bw.write(ss+"\n");
        ...
        bw.write(ss+"\n");
        ss="write(ss ,\""+out_folder+"mkTWAS.result\")";
        bw.write(ss+"\n");
        bw.close();
	}
...
//					ss_eqtl= rs_dict.get(pos)+"\t1.0";
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		if(args.length==0){
			...
			System.out.println("mkTWAS: Disentangling genetic feature selection and aggregation "
					...
					+ "-eqtl_info_path INPUT_EQTL_INFORMATION_FILE\n"
		...
		if(args[0].equals("mkTWAS")){
			final DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss");
			if(args.length < 14){
				System.out.println("Disentangling genetic feature selection and aggregation "
						+ "in transcriptome-wide association studies.\n Usage:\nmkTWAS\n"
						+ "-format csv|plink\n"
						+ "-input_genotype INPUT_GENOTYPE_FILE\n"
						+ "-input_phenotype INPUT_PHENOTYPE_FILE\n"
				...
				for(int k=1;k<args.length;k++){
					if(args[k].startsWith("-")){
						...
						else if(args[k].equals("-eqtl_info_path")) info_file=args[k+1];
				...
				if(genotype_file==null){
					...
					System.out.println("Please input the gene eqtl information file!");
					System.exit(0);
				} else if (output_folder==null){
					System.out.println("Please input the output folder path!");
					System.exit(0);
				} else if (rscript==null){
					System.out.println("Please input Rscript binary file path!");
					System.exit(0);
				} else if (gene_id==null){
					System.out.println("Please input ensembl gene id!");
					System.exit(0);
				}else if (plink==null){
					System.out.println("Please input plink binary file path!");
					System.exit(0);
				...
				System.out.println("Running SKAT with eQTL Weights.\t"+
						dtf.format(LocalDateTime.now()));
				write_R(output_folder, phenotype_type);
				run_Skat(output_folder, rscript);
				System.out.println("Running SKAT with eQTL Weights Finished.\t"+
						dtf.format(LocalDateTime.now()));
				System.out.println("=====================================");
